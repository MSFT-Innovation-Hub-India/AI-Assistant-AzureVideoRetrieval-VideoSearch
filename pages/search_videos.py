        ...
        if document.get("documentId") == document_id:
            try:
                sas_token = generate_container_sas(
                    ...
                    + datetime.timedelta(days=1),  # Token valid for 1 hour
                )
            except Exception as e:
            ...
            print("sas url of selected video: ", sas_url)
            return sas_url
...
# Display read-only text in the sidebar with parameters from .env file
st.sidebar.subheader("Azure Configuration")
st.sidebar.text(f"Endpoint: {az_video_indexer_endpoint}")
st.sidebar.text(f"Index Name: {az_video_indexer_index_name}")
st.sidebar.text(f"API Version: {az_video_indexer_api_version}")
st.sidebar.text(f"Storage Account: {az_storage_account_name}")
st.sidebar.text(f"Container Name: {az_storage_container_name}")
...
if st.session_state.search_results is not None:
    results_df = st.session_state.search_results
    st.dataframe(
        results_df[["documentId", "best", "start", "end", "documentKind", "relevance"]],
